...
> `tech4time-backend`, so it keeps the history, the pipeline and the deploy secrets that were
...
- `public/.htaccess` matches `expr=%{REQUEST_URI} =~ m#^/admin(/|$)#`. On the subdomain the URI is `/`, so
